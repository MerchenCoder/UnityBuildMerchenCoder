...
public class WatcherSleep : MonoBehaviour
{
    // 자식 객체들을 관리하기 위한 리스트
    ...
    void Update()
    {
        if (DataManager.Instance.gameStateData.ch2MissionClear[13])
        {
            Debug.Log("경비병이 쓰러져있는 조건을 충족함");
            FlipImageY(true);
            RotateImage();
    ...
    void RotateImage()
    {
        ...
        if (imageObject != null && !rotated)
        {
            ...
            if (charCanvas != null)
            {
                ...
                Transform dialBtn = charCanvas.Find("dialogue_Btn_After_2_14");
                ...
                if (quizBtn != null)
                {
                    ...
                    float newPosX = (float)-10.3;
                    float newPosY = (float)33.4;
                    quizBtn.localPosition = new Vector3(newPosX, newPosY, quizBtn.localPosition.z);
                    dialBtn.localPosition = new Vector3(newPosX, newPosY, dialBtn.localPosition.z);
                    ...
                    dialBtn.localEulerAngles = new Vector3(180, 0, -270);
