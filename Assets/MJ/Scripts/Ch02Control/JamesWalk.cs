...
public class JamesWalk : MonoBehaviour
{
    ...
    float targetX = 92.68f; // 목표 X 좌표
    float speed = 5f; // 이동 속도

    void Update()
    {
        if (GameManager.Instance.CheckPlayProgress("Mission2-4Y") && !isWalking)
        {
    ...
    void Walking()
    {
        ...
        {
            isWalking = false;
            this.transform.localPosition = new Vector3(targetX, -1.3134f, 0f);
