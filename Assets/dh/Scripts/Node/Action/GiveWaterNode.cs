...
public class GiveWaterNode : MonoBehaviour, INode, IFollowFlow
{
    ...
    public IEnumerator Execute()
    {
        ...
        if (player == null)
        {
            ...
            giveWaterAnim = player.GetComponentInChildren<Animator>(true);
        }
        playerActionBubble.SetActive(true);
        //실행시 처리할 로직이 있는 코루틴 호출해주면 된다.
        giveWaterAnim.SetBool("GiveWater", true);
        yield return new WaitForSeconds(2f);
        playerActionBubble.SetActive(false);
        giveWaterAnim.SetBool("GiveWater", false);
        //출력 배열에 반영
        TestManager.Instance.playerOutput.Add(outputStr);
